...
//---
      ...
      if (SellOrder._OrderType == -1 && BuyOrder._OrderType == -1)
      {
         ...
         double BuyStop_SL = NormalizeDouble(Current_PriceChannel_Top - StopLevel * pips2dbl, Digits);
         double BuyStop_TP = NormalizeDouble(Current_PriceChannel_Top + StopLevel * pips2dbl, Digits);

         double SellStop_SL = NormalizeDouble(Current_PriceChannel_Bottom + StopLevel * pips2dbl, Digits);
         double SellStop_TP = NormalizeDouble(Current_PriceChannel_Bottom - StopLevel * pips2dbl, Digits);
...
bool Open_BuyStop(double price)
{
   ...
   if (OrderTradeMode == NJNTRADE_BUYANDSELL || OrderTradeMode == NJNTRADE_ONLYBUY)
     {
      result = OpenBuyStopOrder(price, StopLevel, StopLevel);
     }
   else
      result = true;
...
bool Open_SellStop(double price)
{
   ...
   if (OrderTradeMode == NJNTRADE_BUYANDSELL || OrderTradeMode == NJNTRADE_ONLYSELL)
     {
      result = OpenSellStopOrder(price, StopLevel, StopLevel);
     }
   else
      result = true;
      ...
      if ((UseTimeFilter && IsAppropriateTimeFrame()) || (!UseTimeFilter))
      {
         ...
         if (sellStopConditionPrice && sellStopConditionStopLoss && sellStopConditionTakeProfit)
         {
          if (SellOrder.OpenPrice != Current_PriceChannel_Bottom)
          {
           ...
           ModifySellStopOrder(SellOrder.TicketNumber, Current_PriceChannel_Bottom, StopLevel, StopLevel);
          }
         }
         ...
         buyStopConditionTakeProfit = (BuyStop_TP - Current_PriceChannel_Top >= stopLevelPoint);
          ...
          if (buyStopConditionPrice && buyStopConditionStopLoss && buyStopConditionTakeProfit)
          {
           if (BuyOrder.OpenPrice != Current_PriceChannel_Top)
           {
            ...
            ModifyBuyStopOrder(BuyOrder.TicketNumber, Current_PriceChannel_Top, StopLevel, StopLevel);
           }
          }
...
bool UpdateOrdersOnTimeFilterActivation()
{
   ...
   bool response = false;
    ...
    {
     if (BuyOrder.OpenPrice != Current_PriceChannel_Top)
        response = ModifyBuyStopOrder(BuyOrder.TicketNumber, Current_PriceChannel_Top, StopLevel, StopLevel);
     else 
        response = true;
        
     if (response && (SellOrder.OpenPrice != Current_PriceChannel_Bottom))
        response = ModifySellStopOrder(SellOrder.TicketNumber, Current_PriceChannel_Bottom, StopLevel, StopLevel);
    }
