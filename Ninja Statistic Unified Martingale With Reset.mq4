...
input int  MaxConsecutiveLoss = 0;
...
void MakeNewOrder()
{
 ...
 if (Open[1] > Close[1])
 {
  if (!Invert)
  {
   OpenBuyOrder(StopLevel, _Lots);
  }
  else
  {
   OpenSellOrder(StopLevel, _Lots);
  }
 }
...
bool OpenBuyOrder(double _StopLevel, double _Lots)
{
   int         Ticket = 0,
               Slippage = 3,
               TerminalStopLevel = 0,
...
bool OpenSellOrder(double _StopLevel, double _Lots)
{
   bool        result        = false,
               UseStopLevel  = false;
