...
void MakeNewOrder()
{
 ...
 double _Lots = Lots * MathPow(2, consecutiveLoss);
 
 if (Open[1] > Close[1])
 {
  if (!Invert)
  {
   OpenBuyOrder(StopLoss, TakeProfit, _Lots);
  }
  else
  {
   OpenSellOrder(StopLoss, TakeProfit, _Lots);
  }
 }
...
bool OpenBuyOrder(double _StopLoss, double _TakeProfit, double _Lots)
{
   int         Ticket = 0,
               Slippage = 3,
               TerminalStopLevel = 0,
...
bool OpenSellOrder(double _StopLoss, double _TakeProfit, double _Lots)
{
   bool        result        = false,
               UseStopLevel  = false;
